...
void Aggregation::print2D(const std::vector<std::vector<double>>& tab) {
...
void Aggregation::calculateTemperature() {
	aggregate();
	//[C]/dt
...
bool Aggregation::gauss(const int& n, std::vector<std::vector<double>> AB, std::vector<double>& X) {
	int i, j, k;
	double m, s;
...
void Aggregation::gaussElimination(std::vector<double>& temp,int time_step) {
	for (int i = 0; i < data.nN; i++) {
		Hg[i][data.nN] = temp[i];
	}
	bool ifZero = false;
	...
	std::cout << time_step << "\t" << min << "\t\t" << max<<std::endl;
	
	data.t0 = t1;
}
double Aggregation::searchMax(std::vector<double>& tab) {
...
double Aggregation::searchMin(std::vector<double>& tab) {
